...
import { FileText, Loader2, Download, MessageCircle, Phone, ChevronLeft, ChevronRight, X, ImageIcon } from "lucide-react";
...
export default function Consulta() {
  ...
  const fetchQuote = async () => {
    setLoading(true);
    try {
      const response = await base44.functions.invoke("getConsulta", { id });
  ...
  const isImage = (url) => url.match(/\.(jpg|jpeg|png|webp|gif|bmp)$/i);
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      const dateStr = new Date(quote.created_date || Date.now()).toISOString().slice(0, 10);
    ...
    <div className="min-h-screen bg-[#F9F8F6]">
      ...
      <div className="max-w-[1200px] mx-auto px-5 sm:px-8 lg:px-14 pt-8 pb-16">
        ...
        <div className="mb-8">
          ...
          <p className="text-[13px] text-[#151515]/50">
            {quote.created_date
              ? new Date(quote.created_date).toLocaleString("es-AR", { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })
              : ""}{""}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 items-start">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-white border border-black/8 rounded-sm p-6 sm:p-8">
                        ...
                        {cfg.entrada === "Tengo medidas/fotos" && (
                          <>
                            <div className="flex flex-col">
                              ...
                              <span className="text-[14px] text-[#151515] font-medium">{cfg.ancho ? `${cfg.ancho} cm` : "-"}</span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-[11px] text-[#151515]/40 mb-0.5">Alto</span>
                              <span className="text-[14px] text-[#151515] font-medium">{cfg.alto ? `${cfg.alto} cm` : "-"}</span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-[11px] text-[#151515]/40 mb-0.5">Profundidad</span>
                              <span className="text-[14px] text-[#151515] font-medium">{cfg.profundidad ? `${cfg.profundidad} cm` : "-"}</span>
                            </div>
                          </>
                        )}
              ...
              {quote.comentarios && (
                <div className="mt-5 pt-5 border-t border-black/5">
                  <span className="text-[11px] text-[#151515]/40 block mb-2">Comentarios del cliente</span>
            ...
            {images.length > 0 && (
              <div className="bg-white border border-black/8 rounded-sm p-6 sm:p-8">
                <div className="flex items-center justify-between mb-5">
                  <h2 className="font-mono text-[11px] tracking-[0.16em] uppercase text-[#151515]/40">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {images.map((url, i) => (
                    <button
                      ...
                      className="group relative aspect-square rounded-sm overflow-hidden border border-black/5 bg-[#F9F8F6]"
                    >
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-end justify-start p-2">
                        <span className="opacity-0 group-hover:opacity-100 text-[10px] text-white font-mono bg-black/50 px-2 py-0.5 rounded-full">
                          {i + 1}/{images.length}
                        </span>
                      </div>
                    </button>
                    ...
                    <div key={i} className="flex items-center justify-between gap-3 p-3 rounded-sm border border-black/5 bg-[#F9F8F6]">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full border border-black/10 text-[12px] font-medium text-[#151515] hover:border-[#151515] transition-colors"
                        >
                          <Download size={13} />
            ...
            {otherFiles.length > 0 && (
              <div className="bg-white border border-black/8 rounded-sm p-6 sm:p-8">
                <h2 className="font-mono text-[11px] tracking-[0.16em] uppercase text-[#151515]/40 mb-5">
                ...
                <div className="space-y-2">
                  {otherFiles.map((url, i) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-3 p-3 rounded-sm border border-black/5 bg-[#F9F8F6] hover:border-[#B38B67] transition-colors"
                    >
                      <FileText size={20} className="text-[#B38B67] shrink-0" />
                      <span className="text-[13px] text-[#151515] truncate">{fileName(url)}</span>
                    </a>
      ...
      {lightbox && (
        <div className="fixed inset-0 z-[60] bg-black/90 flex items-center justify-center p-4" onClick={() => setLightbox(null)}>
          ...
          />
          {images.length > 1 && (
            <span className="absolute bottom-4 left-1/2 -translate-x-1/2 font-mono text-[11px] text-white/60 px-3 py-1 rounded-full bg-black/40">
